	  ...
	  const isWelkom = _welkom.includes(anu.jid)
		...
		try {
            ...
            if (!isWelkom) return
             if (anu.action == 'add' && !mem.includes(zee.user.jid)) {     
              ...
              zee.sendMessage(mdata.id, { contentText: `${teks}`, footerText: `Welcome Message By *Krisna*`, buttons: [{buttonId: `.selamatdatang`,buttonText:{displayText: 'WELCOME KAK'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
		}
		...
		if (!isWelkom) return
            if (anu.action == 'remove' && !mem.includes(zee.user.jid)) {
               ...
               buff = await getBuffer(pic)
               zee.sendMessage(mdata.id, { contentText: `${out}`, footerText: `Left Message By *Krisna*`, buttons: [{buttonId: `.bay`,buttonText:{displayText: 'BYE KAK👋'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
            }
			} catch (e) {
